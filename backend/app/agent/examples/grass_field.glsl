// Prompt: "grass blowing in the wind"
// Tags: grass, field, nature, wind, terrain, green, landscape, meadow, lawn, ground, vegetation, plant
#version 300 es
...
// Individual grass blade shape + color
vec4 grass(vec2 p, float x){
  float s = mix(0.7, 2.0, 0.5 + sin(x * 12.0) * 0.5);
  p.x += pow(1.0 + p.y, 2.0) * 0.1 * cos(x * 0.5 + iTime);
  p.x *= s;
  p.y = (1.0 + p.y) * s - 1.0;
  float m = 1.0 - smoothstep(0.0, clamp(1.0 - p.y*1.5, 0.01, 0.6)*0.2*s,
                              pow(abs(p.x)*19.0, 1.5) + p.y - 0.6);
  return vec4(mix(vec3(0.05,0.1,0.0)*0.8, vec3(0.0,0.3,0.0), (p.y+1.0)*0.5+abs(p.x)),
...
vec3 backg(vec3 ro, vec3 rd){
  ...
  vec3 ground = mix(horiz, vec3(0.04, 0.07, 0.0)*0.6,
                    pow(max(0.0, dot(rd, vec3(0.0,-1.0,0.0))), 0.2));
...
void main(){
  ...
  for(int i = 0; i < BLADES; i++){
    ...
    fcol = mix(fcol, c.rgb, step(0.0, pt) * c.w);
  }
  ...
  fragColor = vec4(fcol * 1.8 + vec3(dither()), 1.0);
}
